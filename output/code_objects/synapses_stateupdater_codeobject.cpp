...
const int _numpost = _dynamic_array_synapses_post.size();
 ...
 const double _lio_1 = exp((- dt) / 0.02);
 const double _lio_2 = exp((- dt) / 0.02);
	...
	for(int _idx=0; _idx<_N; _idx++)
	{
        ...
        const double _pre = _lio_1 * pre;
        const double _post = _lio_2 * post;
        pre = _pre;
        post = _post;
    ...
    synapses_stateupdater_codeobject_profiling_info += _run_time;
}
