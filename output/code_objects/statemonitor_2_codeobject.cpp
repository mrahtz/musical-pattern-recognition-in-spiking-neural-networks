...
const int _num_source_post = _dynamic_array_synapses_post.size();
 ...
 double*   _ptr_array_statemonitor_2_clock_t = _array_statemonitor_2_clock_t;
 ...
 double* __restrict  _ptr_array_synapses_post = _array_synapses_post;
    ...
    _dynamic_array_statemonitor_2_t.push_back(_ptr_array_statemonitor_2_clock_t[0]);

    const int _new_size = _dynamic_array_statemonitor_2_t.size();
    // Resize the dynamic arrays
    _dynamic_array_statemonitor_2_post.resize(_new_size, _num_indices);
    _dynamic_array_statemonitor_2_pre.resize(_new_size, _num_indices);
    _dynamic_array_statemonitor_2_w.resize(_new_size, _num_indices);
    ...
    {
        // vector code
        const int _idx = _ptr_array_statemonitor_2__indices[_i];
        const int _vectorisation_idx = _idx;
                                        
                    const double _source_pre = _ptr_array_synapses_pre[_idx];
                    const double _source_w = _ptr_array_synapses_w[_idx];
                    const double _source_post = _ptr_array_synapses_post[_idx];
                    const double _to_record_w = _source_w;
                    const double _to_record_post = _source_post;
                    const double _to_record_pre = _source_pre;



            _dynamic_array_statemonitor_2_post(_new_size-1, _i) = _to_record_post;
            _dynamic_array_statemonitor_2_pre(_new_size-1, _i) = _to_record_pre;
            _dynamic_array_statemonitor_2_w(_new_size-1, _i) = _to_record_w;
    }
    ...
    statemonitor_2_codeobject_profiling_info += _run_time;
}
